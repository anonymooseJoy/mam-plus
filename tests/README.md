...
This repo now has two automated test layers that are safe to commit:
...
All fixtures in this directory are synthetic. They are hand-written HTML pages that model only the DOM each feature needs. No captured MAM HTML, account data, or credentials are required.
